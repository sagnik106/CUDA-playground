...
{   
    ...
    printf("CPU sum : %d \nGPU sum : %d\n",cpu, arr[0]);
    printf(cpu==arr[0]?"CPU and GPU values Match\n":"CPU and GPU values do not match\n");
    printf("GPU memory transfer time : %lf sec\n",(double)((-gt2+gt1+gtt2-gtt1)/(double)CLOCKS_PER_SEC));
    printf("GPU calculation time : %lf sec\n",(double)((gt2-gt1)/(double)CLOCKS_PER_SEC));
    printf("CPU calculation time : %lf sec\n",(double)((ct2-ct1)/(double)CLOCKS_PER_SEC));
